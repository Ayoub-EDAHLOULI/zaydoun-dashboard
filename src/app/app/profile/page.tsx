...
import { BookOpen, MessageSquare, Pencil, Calendar, Lock, Eye, EyeOff, Loader2 } from "lucide-react";
import { z } from "zod";
...
function StatCard({ icon: Icon, label, value }: { icon: React.ElementType; label: string; value: number }) {
  return (
    <div
      ...
      style={{ backgroundColor: "var(--z-bg-elevated)", border: "1px solid var(--z-border)" }}
    >
      <div
      ...
      <div>
        <p className="text-2xl font-bold" style={{ color: "var(--z-text-primary)" }}>{value}</p>
        <p className="text-xs mt-0.5" style={{ color: "var(--z-text-muted)" }}>{label}</p>
      </div>
    </div>
  );
}

function ChangePasswordSection({ accessToken }: { accessToken: string | null }) {
  ...
  const set = (field: keyof PasswordFormData) => (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const focusHandlers = (field: keyof PasswordFormData) => ({
    onFocus: (e: React.FocusEvent<HTMLInputElement>) => {
      e.currentTarget.style.borderColor = errors[field] ? "var(--z-error)" : "var(--z-gold)";
  ...
  const inputBase = "w-full pl-10 pr-10 py-2.5 rounded-xl text-sm outline-none transition-all duration-150";

  const fields: { key: keyof PasswordFormData; label: string; showKey: keyof typeof show; placeholder: string }[] = [
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2.5 mb-5">
        ...
        <div>
          <h3 className="text-sm font-semibold" style={{ color: "var(--z-text-primary)" }}>
            Change password
          </h3>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        {fields.map(({ key, label, showKey, placeholder }) => (
          <div key={key}>
            <label className="block text-sm font-medium mb-1.5" style={{ color: "var(--z-text-secondary)" }}>
              {label}
            </label>
            ...
            {errors[key] && (
              <p className="text-xs font-medium mt-1.5" style={{ color: "var(--z-error)" }}>
                {errors[key]}
              </p>
            )}
          </div>
        ...
        <div className="flex justify-end pt-1">
          <button
            ...
            style={{ backgroundColor: "var(--z-gold)", color: "#0d0d0d", minWidth: "9rem" }}
            onMouseEnter={(e) => {
              if (!isLoading) e.currentTarget.style.backgroundColor = "var(--z-gold-light)";
            }}
            onMouseLeave={(e) => {
...
export default function UserProfilePage() {
  ...
  const handleSave = async ({ name }: { name: string }) => {
    try {
      ...
      setProfile(updated);
      updateUser({ id: updated.id, name: updated.name ?? "", email: updated.email, role: "USER" });
    ...
    <>
      ...
      <div className="max-w-2xl space-y-6">
        ...
        >
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-lg font-semibold" style={{ color: "var(--z-text-primary)" }}>
                  {profile.name ?? profile.email}
                </h2>
                <p className="text-sm" style={{ color: "var(--z-text-muted)" }}>{profile.email}</p>
                {joinedDate && (
                  <div className="flex items-center gap-1.5 mt-1.5" style={{ color: "var(--z-text-muted)" }}>
            ...
            <button
              ...
              onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = "rgba(201,168,76,0.25)")}
              onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = "var(--z-gold-muted)")}
            >
        ...
        <div className="grid grid-cols-2 gap-4">
          <StatCard icon={BookOpen} label="Books in library" value={profile.stats.totalBooks} />
          <StatCard icon={MessageSquare} label="Conversations" value={profile.stats.totalConversations} />
        </div>
